 ...
 * @author m057188
 * 
 * Provides pdf document creation functionality
 * optional functionality of watermark is handled by 
 * pdfEventHandler
 * 
 */
public class PdfDocWriter implements DocWriter {
	...
	public void addCell(String value, Map<String, String> attrs){
		...
		/*if(attrs.containsKey("editable")){
			...
			String editable = attrs.get("editable");
			//textfield
			if(editable.equals("text")){
				//create new text field
				TextField textfield = pdfFormHelper.createTextField(writer, attrs.get("name"), attrs.containsKey("required"));
				
				//use cell event to include textfield in the cell
				cell.setMinimumHeight(10);
				try {
					cell.setCellEvent(new FieldCell(textfield.getTextField(), 200, writer));
				} catch (IOException e) {
					log.error("error in adding textfield", e);
				} catch (DocumentException e) {
					log.error("error in adding textfield", e);
				}
			}
		...
		colIndex+=cols;
		//increment row and col indexes (this is only used for setting header rows)
	...
	//internal function to figure out printing paper dimensions
