...
public class ExcelDocWriter implements DocWriter {
	...
	private NumberFormat currencyNumberFormat = new NumberFormat(NumberFormat.CURRENCY_DOLLAR + "#,###",
	...
	public void init(Map<String, String> reportAttrs) throws DocumentCreationException {
		...
		try {
			...
			log.error("error in creating document", e);
	...
	public void createTable(int cols, String type) {
		if (!tableIndexesOverride) {
			// rowIndex = 0; rowIndex needs to keep incrementing because
	...
	public void addCell(String value, Map<String, String> attrs) throws DocumentDataEntryException {
		...
		try {
			...
			if ((dataType.equals("number") || dataType.equals("percent")) && !"".equals(value)) {
				double val = dataType.equals("number") ? Double.parseDouble(value) : Double.parseDouble(value
						.replaceAll("%", "")) / 100;
				// add number to cell
			...
			// colspan (default to 1 to increment atleast on column, as we have
			// added one)
			int colspan = 1;
